...
size<-200
for(i in 1:15){
  assign(paste("demand_", i, sep = ""), rnorm(size,mean[i],std[i]))
}
```
...
mac[8,]<-c(solution[13:15],optimal_profit)
df<-data.frame(mac)
kable(df,format='latex', booktabs = T,align = 'c') %>% 
        kable_styling(latex_options = "scale_down")
```
...
prilower_limit<-0.9*(pridiff[,10]+pridiff[,11])/2
priupper_limit<-1.1*(pridiff[,10]+pridiff[,11])/2
prilower_record<-c(1,3,10,5,19,6,7,3,11,2,10,4,6,10,6)
priupper_record<-c(1,2,15,6,14,5,9,3,9,2,5,3,5,3,5)
```
