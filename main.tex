...
In Figure \ref{fig:con}, two example boundaries are used to demonstrate different kinds of feasible region (on the left side of boundary). Moreover, we define a \emph{optimum point} to denote the optimal solution when all constraints are relaxed. One can see that boundary 1 forms a feasible region contains the optimum point, while the feasible region formed by boundary 2 doesn't. There is no doubt that the optimal solution for MPNP with boundary 1 is lying exactly at the optimum point, and the solution will remains optimal as long as the optimum point is not 'pushed out' of the feasible region. On the other hand, one can easily show the optimal solution for MPNP with boundary 2 lies on the boundary \cite{DT06}, and the solution on this case is more sensitive. Cost function on boundaries see Figure \ref{fig:bd1} and Figure \ref{fig:bd2} in Appendix \ref{se:costfun}.

Now, we consider the boundary for example in Section \ref{se:sensitivity}
...
\section{Cost function on boundaries}
\label{se:costfun}

\begin{figure}[ht]
\centering
\caption{Cost function on boundary 1}
...
\begin{figure}[ht]
\centering
\caption{Cost function on boundary 2}
