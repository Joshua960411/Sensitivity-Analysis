...
We consider one complicated information intakes now, where two or more correlated parameters are changed simultaneously.
...
\begin{aligned}
    \Delta \, C_{c_a^u}
    & = c_a^o \Delta y_a + \sum_{s \in S} \big[ \big( c_a^u + \Delta c_a^u \big) \big( z_a^s + \Delta z_a^s \big) - c_a^u z_a^s \big]\\
    & = -c_a^o N_a^o \Delta \mu_a + c_a^u 
    N_a^u \Delta \mu_a + \Delta c_a^u \sum_{s \in S} z_a^s + N_a^u \Delta c_a^u \Delta \mu_a\\
    & = -c_a^o N_a^o \beta \Delta c_a^u + c_a^u N_a^u \beta \Delta c_a^u + \Delta c_a^u \sum_{s \in S} z_a^s + N_a^u \Delta c_a^u \beta \Delta c_a^u\\
    & = N_a^u \beta \big( \Delta c_a^u \big)^2 + \big[ c_a^u N_a^u \beta - c_a^o N_a^o \beta + \sum_{s \in S} z_a^s \big] \Delta c_a^u
\end{aligned}
\]
One can see there exists a parabolic relationship between $\Delta \, C_{c_a^u}$ and $\Delta c_a^u$. Thus, we may apply features of parabolic function:
\begin{itemize}
    \item Point $(0,0)$ is on the parabola,
    \item Given $\beta < 0$, the parabola is opening to the bottom,
    \item The axis of parabola is positive when $\sum_{s \in S} z_a^s > \beta \big( c_a^o N_a^o - c_a^u N_a^u \big)$.
\end{itemize}
Using these features, we could derive:
\begin{eqnarray}
\label{eq:condition}
    \frac{1}{|S|} \sum_{s \in S} z_{j'}^s > \frac{\beta \big( c_{j'}^o N_{j'}^o - c_{j'}^u N_{j'}^u \big)}{N_{j'}^o + N_{j'}^u} \implies \lim_{\Delta c_{j'}^u \to 0^-} \frac{\Delta \, C_{x^*}}{\Delta c_{j'}^u} > 0
\end{eqnarray}
