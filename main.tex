...
\begin{eqnarray*}
	...
	& y^s, z^s \in \RR_+^n			& (s \in S).
\end{eqnarray*}
It has been shown very difficult to deal with joint perturbations on both objective function coefficient and constraint RHS mathematically. Thus, here we only consider the case when current optimal solution $\big( x^*,y^*,z^* \big)$ remains optimal. We first define:
\[
    \Delta y_j^s =
    \begin{cases}
        -\Delta \mu_j, & y_j^s > 0,\\
