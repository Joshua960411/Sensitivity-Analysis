...
fig_con <- plot_ly(x= ~x_1,y= ~x_2,z= ~cost, type = "contour"
  ...
  add_segments(x = 150, xend = 180, y = 236, yend = 228,color = I("blue"),name = 'Boundary 2',size = I(1)) %>%
  add_segments(x = 180, xend = 197, y = 228, yend = 216,color = I("blue"),showlegend = FALSE,size = I(1)) %>%
  add_segments(x = 197, xend = 206, y = 216, yend = 204,color = I("blue"),showlegend = FALSE,size = I(1)) %>%
  add_segments(x = 206, xend = 214, y = 204, yend = 185,color = I("blue"),showlegend = FALSE,size = I(1)) %>%
  add_segments(x = 214, xend = 220, y = 185, yend = 150,color = I("blue"),showlegend = FALSE,size = I(1)) %>%
  add_trace(x = min_ind[1,1], y = min_ind[1,2], type = "scatter",name = 'optimum',color=I("Orange")) %>%
  config(
    toImageButtonOptions = list(
      format = "png",
      filename = "contour",
...
fig_bou2 <- plot_ly(x = ~xq, y = ~yq, z = ~zq, type = 'scatter3d', mode = 'lines',
        line = list(width = 6, colorscale = 'Viridis')) %>% layout(
    scene = list(
      xaxis = list(title = "x_1"),
      yaxis = list(title = "x_2"),
      zaxis = list(title = "cost"),
...
fig_conexam <- plot_ly(x= ~x_1,y= ~x_2,z= ~cost, type = "contour"
               ...
               line = list(smoothing = 1))%>%
  add_segments(x = 175, xend = 181.8, y = 250, yend = 245.5,color = I("red"),name = 'Boundary',size = I(1)) %>%
  add_segments(x = 181.8, xend = 250, y = 245.5, yend = 150,color = I("red"),showlegend = FALSE,size = I(1))%>%
  add_trace(x = min_ind[1,1], y = min_ind[1,2], type = "scatter",name = 'optimum',color=I("Orange"))%>%
  add_trace(x = optimum_original$solution[49], y = optimum_original$solution[50], type = "scatter",name = 'optimum(with boundary)',color=I("red"))%>%
  config(
    toImageButtonOptions = list(
      format = "png",
      filename = "conexam",
