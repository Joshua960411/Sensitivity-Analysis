...
fig_main <- fig_main %>% layout(
    title = "Layout costs in a 3D plot",
    scene = list(
  ...
  add_trace(x = optimum_original$solution[49], y = optimum_original$solution[50]
            ,z = optimum_original$objval,type = 'scatter3d'
            ,name = 'optimum')
...
fig_con <- plot_ly(x= ~x_1,y= ~x_2,z= ~cost, type = "contour"
  ...
  layout(title = "Contour of costs with boundary") %>% 
  add_trace(x = optimum_original$solution[49], y = optimum_original$solution[50], type = "scatter",name = 'optimum')
